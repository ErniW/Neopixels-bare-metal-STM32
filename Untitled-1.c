/*
   ...
   This code written for a bluepill (stm32f103c8 china board) uses the
   fabooh core for arduino.
   Author: rick kimball
   Log: 
...
static const uint32_t HZ=800000;
...
LED_BUILTIN led_pin;              // built-in PC13 pin of bluepill
PA1 ws281x_pin;                   // PA1 outputs WS281x pulse train connect to DIN
//----------------------------------------------------------------------
...
//
void setup() {
  // enable bluepill built-in led (PC13)
  led_pin.setmode_output();
  led_pin.off();
  // set PA1 to Push Pull Alternate IO
  ws281x_pin.setmode_pwm();
}
void loop() {
...
void on_dma_complete()
{
  ...
  // turn off DMA CH7
  DMA1_Channel7->CCR &= ~DMA_CCR_EN;
  led_pin.toggle(); // toggle builtin_led
}
#if F_CPU < 20000000UL
 #error "20MHz is minimum speed for success"
#endif